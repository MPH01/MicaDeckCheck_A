pycode = "Mica DeckCheck_v0.02.py"
...
#Versions
#   v0.01 - Initial setup
#   v0.02 - Change dropdown to listbox with multi-selection then RMB to acknowledge selection
#       Was able to make a relatively generic DEF for Listbox with selection. 
#   v0.03 - ?
...
Txt_Srch_Props = ("All Properties","PSHELL","PCOMP","PBAR","PBEAM","PSOLID","PBUSH","PSHEAR","PSHLN1","PSHLN2","PSLDN1")
Txt_Srch_Elms = ("All Elements","CBAR","CBEAM","CTRIA","CQUAD","CTETRA","CHEXA","CPENTA")
Txt_Srch_LBCs = ("All LBCs","SPC","LOAD","PLOAD1","PLOAD4","TEMP","TEMPERATURE")
...
MbVar1  = IntVar()
MbVar2 = IntVar()
PrefMenu.add_checkbutton ( label="Option1", variable=MbVar1 )
PrefMenu.add_separator()
PrefMenu.add_checkbutton ( label="Option2",variable=MbVar2 )
...
#PropListBox.bind("<Button-3>",EndMe )
#PropListBox = OptionMenu( root,PropVar,*Txt_Srch_Props)#, command=PayeeEntryBoxC() )
#PropListBox.grid(row=3,column=2, sticky = W)

        #Properties Variable for Listbox
PropVar = StringVar()
PropVar.set(Txt_Srch_Props[0])

def PropList0():
    PropFrame = Toplevel(width=100, height=300)
    def EndMe(event):
        PropFrame.quit()
        print "RMB?"
    PropListBox = Listbox(PropFrame, selectmode = MULTIPLE)    #height = 9,SINGLE,EXTENDED
    j=1
    for i in Txt_Srch_Props:
        PropListBox.insert(j, i)
        j+=1
    PropListBox.bind("<Button-3>",EndMe )
    PropListBox.pack()
    PropFrame.mainloop()
    print "List Box Pick:"
    print str( PropListBox.curselection() )
    PropSelection = ''
    for k in PropListBox.curselection():
        PropSelection += Txt_Srch_Props[int(k)]+", "
    PropSelection = PropSelection[:-2]
    print PropSelection
    PropVar.set(PropSelection)
    PropFrame.destroy()
    
bProperties = Button(root,width=15, text='Properties',command=PropList0)
bProperties.grid(row=3,column=0, sticky = W)
def PropertiesHelpInfo():
    tkMessageBox.showinfo(title = "Property Info", message = PropertiesHelpInfoMSG)
...
#Txt_Srch_Elms
        #Element Variable for Listbox
ElmVar = StringVar()
ElmVar.set(Txt_Srch_Elms[0])

def GenListWthPick0(LvarIn,GvarOut):
    gFrame = Toplevel(width=100, height=300)
    def EndMe(event):
        gFrame.quit()
        print "RMB?"
    L_Box = Listbox(gFrame, selectmode = MULTIPLE)    #height = 9,SINGLE,EXTENDED
    j=1
    for i in LvarIn:
        L_Box.insert(j, i)
        j+=1
    L_Box.bind("<Button-3>",EndMe )
    L_Box.pack()
    gFrame.mainloop()
    print "List Box Pick:"
    print str( L_Box.curselection() )
    uSelection = ''
    for k in L_Box.curselection():
        uSelection += LvarIn[int(k)]+", "
    uSelection = uSelection[:-2]
    print uSelection
    GvarOut.set(L_Box.curselection())
    gFrame.destroy()

def DummyProp():
    GenListWthPick0(Txt_Srch_Elms,ElmVar)
    
#bElements = Button(root,width=15, text='Elements',command=ElmList0) #works
#bElements = Button(root,width=15, text='Elements',command=GenListWthPick0(Txt_Srch_Elms,ElmVar) ) #pre-pop
bElements = Button(root,width=15, text='Elements',command=DummyProp ) #works
bElements.grid(row=4,column=0, sticky = W)
...
root.mainloop()
print "Prop Selection (Post-Quit): "+str( PropVar.get() )
print "Element Selection (Post-Quit): "+str( ElmVar.get() )
root.destroy()
